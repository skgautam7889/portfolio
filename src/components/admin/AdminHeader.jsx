...
const Header = () => {
  ...
  return (
    <>
      <header className={`header ${scrolled ? "header--scrolled" : ""}`}>
        <div className="container">
          <Link to="/" className="logo">
            <img
              src={data.website?.logo || "/images/logo/logo.svg"}
              alt={data.website?.name || "Logo"}
            />
            <span>{data.website?.first_name || "Portfolio"}</span>
            <span className="last_name">
              {data.website?.last_name || "Portfolio"}
            </span>
          </Link>
      ...
      {/* Mobile Offcanvas */}
      <div
        className={`offcanvas-overlay ${mobileOpen ? "offcanvas-overlay--open" : ""}`}
